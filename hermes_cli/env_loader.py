...
def load_hermes_dotenv(
    *,
    ...
    # ...) already resolved during a previous load_hermes_dotenv() call in
    # this process.  ``load_dotenv(override=True)`` below would otherwise
    # write the raw .env placeholder (e.g. ``__BITWARDEN_MANAGED__``) back
    # over the resolved value, and ``_apply_external_secret_sources()`` is an
