...
class CommandDef:
    ...
    # Classic-CLI dispatch.  Names a ``HermesCLI`` method taking the raw
    # command string (``cmd_original``); ``process_command`` resolves it with
    # ``getattr`` instead of matching the name in a hand-written elif chain,
    # so registering a command and dispatching it are one edit.  A string key
    # (not a callable) keeps this module import-light, matching ``execute``
    # and ``busy_handler``.  Unlike ``execute``, the target may mutate agent
    # and session state and is CLI-specific.
    cli_handler: str | None = None
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("prompt", "Compose your next prompt in $EDITOR (markdown), then send it", "Session",
               ...
               cli_handler="_handle_prompt_compose_command"),
    ...
    CommandDef("branch", "Branch the current session (explore a different path)", "Session",
               aliases=("fork",), args_hint="[name]", cli_handler="_handle_branch_command"),
    CommandDef("compress", "Compress conversation context (add 'here [N]' to keep recent N turns; --preview shows what would happen)", "Session",
               ...
               cli_handler="_manual_compress"),
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]", cli_handler="_handle_rollback_command"),
    CommandDef("snapshot", "Create or restore state snapshots of Hermes config/state", "Session",
               cli_only=True, aliases=("snap",), args_hint="[create|restore <id>|prune]",
               cli_handler="_handle_snapshot_command"),
    ...
    CommandDef("background", "Run a prompt in the background", "Session",
               ...
               cli_handler="_handle_background_command"),
    ...
    CommandDef("journey", "Open the learning journey timeline",
               ...
               subcommands=("list", "delete", "edit"), cli_handler="_handle_journey_command"),
    ...
    CommandDef("goal", "Set a standing goal Hermes works on across turns until achieved", "Session",
               ...
               busy_policy="dispatch", busy_handler="goal", cli_handler="_handle_goal_command"),
    ...
    CommandDef("subgoal", "Add or manage extra criteria on the active goal", "Session",
               ...
               cli_handler="_handle_subgoal_command"),
    ...
    CommandDef("context", "Show detailed context window view with usage gauge, category breakdown, compression stats, and throughput", "Session",
               ...
               busy_policy="dispatch", cli_handler="_show_context_breakdown"),
    ...
    CommandDef("resume", "Resume a previously-named session", "Session",
               args_hint="[name]", cli_handler="_handle_resume_command"),

    # Configuration
    CommandDef("sessions", "Browse and resume previous sessions", "Session",
               cli_handler="_handle_sessions_command"),
    ...
    CommandDef("model", "Switch model (session-scoped; --global to persist)", "Configuration",
               ...
               busy_policy="reject", busy_handler="model", cli_handler="_handle_model_switch"),
    CommandDef("codex-runtime", "Toggle codex app-server runtime for OpenAI/Codex models",
               "Configuration", aliases=("codex_runtime",),
               args_hint="[auto|codex_app_server]",
               busy_policy="reject", busy_handler="codex-runtime", cli_handler="_handle_codex_runtime"),

    # /personality receives the original case; the handler lowercases the name itself.
    CommandDef("personality", "Set a predefined personality", "Configuration",
               args_hint="[name]", cli_handler="_handle_personality_command"),
    ...
    CommandDef("focus", "Toggle focus view — show only your prompt and the final response",
               "Configuration", cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status"), cli_handler="_handle_focus_command"),
    CommandDef("footer", "Toggle gateway runtime-metadata footer on final replies",
               ...
               cli_handler="_handle_footer_command"),
    ...
    CommandDef("approvals", "Show or set the persistent dangerous-command approval mode",
               ...
               subcommands=("manual", "smart", "off"), cli_handler="_handle_approvals_command"),
    CommandDef("reasoning", "Manage reasoning effort and display", "Configuration",
               ...
               cli_handler="_handle_reasoning_command"),
    CommandDef("fast", "Toggle fast mode — OpenAI Priority Processing / Anthropic Fast Mode (Normal/Fast)", "Configuration",
               args_hint="[normal|fast|status] [--global]",
               subcommands=("normal", "fast", "status", "on", "off", "--global"),
               cli_handler="_handle_fast_command"),
    CommandDef("skin", "Show or change the display skin/theme", "Configuration",
               cli_only=True, args_hint="[name]", cli_handler="_handle_skin_command"),
    CommandDef("indicator", "Pick the TUI busy-indicator style", "Configuration",
               cli_only=True, args_hint="[kaomoji|emoji|unicode|ascii]",
               subcommands=("kaomoji", "emoji", "unicode", "ascii")),
    CommandDef("voice", "Toggle voice mode", "Configuration",
               args_hint="[on|off|tts|status]", subcommands=("on", "off", "tts", "status"),
               cli_handler="_handle_voice_command"),
    CommandDef("wake", "Toggle the 'Hey Hermes' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status"), cli_handler="_handle_wake_command"),
    CommandDef("busy", "Control what Enter does while Hermes is working", "Configuration",
               cli_only=True, args_hint="[queue|steer|interrupt|status]",
               subcommands=("queue", "steer", "interrupt", "status"), cli_handler="_handle_busy_command"),

    # Tools & Skills
    CommandDef("tools", "Manage tools: /tools [list|disable|enable] [name...]", "Tools & Skills",
               args_hint="[list|disable|enable] [name...]", cli_only=True, cli_handler="_handle_tools_command"),
    ...
    CommandDef("memory", "Review pending memory writes / toggle the approval gate",
               ...
               subcommands=("pending", "approve", "reject", "approval"), cli_handler="_handle_memory_command"),
    CommandDef("bundles", "List skill bundles (aliases /<name> for multiple skills)",
               "Tools & Skills", execute="bundles", cli_handler="_handle_bundles_command"),
    CommandDef("pet", "Toggle or adopt a petdex mascot (/pet, /pet list, /pet <slug>)", "Tools & Skills",
               ...
               cli_handler="_handle_pet_command"),
    CommandDef("hatch", "Generate a new petdex pet from a description",
               ...
               cli_handler="_handle_hatch_command"),
    CommandDef("learn", "Learn a reusable skill from anything you describe (dirs, URLs, this chat, notes)",
               "Tools & Skills", args_hint="<what to learn from>", cli_handler="_handle_learn_command"),
    CommandDef("init", "Generate or update AGENTS.md project instructions from a repo scan",
               "Tools & Skills", args_hint="[notes]", cli_handler="_handle_init_command"),
    CommandDef("cron", "Manage scheduled tasks", "Tools & Skills",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("list", "add", "create", "edit", "pause", "resume", "run", "remove"),
               cli_handler="_handle_cron_command"),
    ...
    CommandDef("kanban", "Multi-profile collaboration board (tasks, links, comments)",
               ...
               busy_policy="dispatch", cli_handler="_handle_kanban_command"),
    CommandDef("reload", "Reload .env variables into the running session", "Tools & Skills",
               cli_only=True),
    # /reload-mcp is the interactive path: _confirm_and_reload_mcp confirms first (unless
    # opted out).  The file-watcher auto-reload calls _reload_mcp directly, no confirm.
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",), cli_handler="_confirm_and_reload_mcp"),
    ...
    CommandDef("browser", "Connect browser tools to your live Chromium-family browser via CDP", "Tools & Skills",
               ...
               subcommands=("connect", "disconnect", "status"), cli_handler="_handle_browser_command"),
    ...
    CommandDef("usage", "Show token usage and rate limits; `reset` redeems a banked Codex limit reset", "Info",
               args_hint="[reset [--force]]", cli_handler="_handle_usage_command"),
    ...
    CommandDef("topup", "Show your Nous balance and manage billing on the portal", "Info",
               cli_handler="_show_billing"),
    CommandDef("insights", "Show usage insights and analytics", "Info",
               args_hint="[days]", cli_handler="_show_insights"),
    ...
    CommandDef("copy", "Copy the last assistant response to clipboard", "Info",
               cli_only=True, args_hint="[number]", cli_handler="_handle_copy_command"),
    CommandDef("paste", "Attach clipboard image from your clipboard", "Info",
               cli_only=True),
    CommandDef("image", "Attach a local image file for your next prompt", "Info",
               cli_only=True, args_hint="<path>", cli_handler="_handle_image_command"),
    CommandDef("update", "Update Hermes Agent to the latest version", "Info",
               busy_policy="dispatch"),
    CommandDef("version", "Show Hermes Agent version", "Info", aliases=("v",),
               busy_policy="dispatch", execute="version"),
    CommandDef("debug", "Upload debug report (system info + logs) and get shareable links", "Info",
               args_hint="[nous|local]", cli_handler="_handle_debug_command"),
