...
class TestSteerMarkerContract:
    ...
    def test_note_has_no_fillable_marker_template(self):
        """Regression (#65339): rendering the marker as a fillable template
        (open marker, ``<their message>`` placeholder, close marker) handed the
        model a copyable example, and it fabricated marker blocks in its own
        output.  The note must describe the markers without composing them
        into a fill-in-the-blank form."""
        from agent.prompt_builder import (
            STEER_CHANNEL_NOTE,
            STEER_MARKER_CLOSE,
        )

        assert "<their message>" not in STEER_CHANNEL_NOTE
        fillable_form = f"{STEER_MARKER_OPEN}\n"
        assert not (
            fillable_form in STEER_CHANNEL_NOTE
            and STEER_MARKER_CLOSE in STEER_CHANNEL_NOTE.split(fillable_form, 1)[-1][:80]
        ), "note must not compose open/placeholder/close into a template"

    def test_note_forbids_self_generation_of_the_marker(self):
        """Regression (#65339): the note must explicitly tell the model that
        the marker is system-produced and never to be generated by the model
        itself — the trust anchor only works if it cannot be self-issued."""
        from agent.prompt_builder import STEER_CHANNEL_NOTE

        assert "Never generate" in STEER_CHANNEL_NOTE
        assert "only produced by the system" in STEER_CHANNEL_NOTE
