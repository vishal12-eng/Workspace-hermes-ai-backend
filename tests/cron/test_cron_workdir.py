...
class TestRunJobTerminalCwd:
    ...
    def test_workdir_sets_and_restores_terminal_cwd(
        ...
        assert observed["resolved_cwd_during_init"] == str(tmp_path.resolve())
        assert observed["resolved_cwd_during_run"] == str(tmp_path.resolve())
        assert observed["authoritative_cwd_during_init"] == str(tmp_path.resolve())
        assert observed["authoritative_cwd_during_run"] == str(tmp_path.resolve())
    ...
    def test_no_workdir_leaves_terminal_cwd_untouched(self, monkeypatch):
        ...
        assert observed["resolved_cwd_during_init"] == before
        assert observed["authoritative_cwd_during_init"] is None
