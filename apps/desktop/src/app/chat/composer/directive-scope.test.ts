...
function paste(editor: HTMLDivElement, text: string) {
  insertComposerContentsAtCaret(editor, pathifyRefs(linkifyUrls(text)), openDirectiveScope(editor)?.length ?? 0)
}
...
describe('directive scope is a browse mode, not text to maintain', () => {
  ...
  it('openDirectiveScope only fires on an EMPTY scope', () => {
    expect(openDirectiveScope(typed('@url:'))).toMatchObject({ scope: 'url', length: 5 })
    expect(openDirectiveScope(typed('@url:https://x.com'))).toBeNull()
    expect(openDirectiveScope(typed('plain text'))).toBeNull()
  })
